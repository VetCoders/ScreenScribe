...
def main():
    ...
    html_content = render_html_report_pro(
        ...
        segments=segments,
        errors=[
            {"stage": "vision_analysis", "message": "VLM endpoint niedostępny, pominięto analizę wizualną"},
        ],
        embed_video=False,
    )

    # Write to file
    output_path = Path("/tmp/screenscribe-test-report.html")
    ...
    print(f"  ✓ Pro template renders")
    print(f"  ✓ Executive summary included")
    print(f"  ✓ Statistics cards rendered")
    print(f"  ✓ Severity badges (critical, high, medium, low)")
    print(f"  ✓ Unified analysis fields present")
    print(f"  ✓ Human review sections")
    print(f"  ✓ Subtitle sync sidebar")
    print(f"  ✓ Error section")
    print(f"  ✓ Footer and scripts embedded")
